...
# If exasol credentials are available then set these
if config['exasol']:
    host = config['exasol']['host']
    port = config['exasol']['port']
    uid = config['exasol']['user']
    pwd = config['exasol']['password']
    schema = config['exasol']['schema']
    table = config['exasol']['table']
    websocket_str = 'ws://'+ host + ':' + port
...
sql = 'INSERT INTO APPMONITOR.APPLICATION_EVENTS ("username", "application", "event_type", "event_timestamp") VALUES ({}, {}, {}, {});'.format(quote_str(username_var), quote_str(application_var), quote_str(event_type_var), quote_str(currenttime_var))

print(sql)

# data = [
#     (username_var, application_var, event_type_var, currenttime_var)
# ]
